...
# === Server Interface ===
class RTServer:
    def __init__(self, rt_path: Union[str, Path]) -> Path:
        """
    ...
    def predict_action(self, payload: Dict[str, Any]) -> str:
        try:
            ...
            print("action", action)
            actions = np.concatenate([action["world_vector"], action["rot_axangle"], action["gripper"]])
            # if double_encode:
...
class DeployConfig:
    # fmt: off
    rt_path: Union[str, Path] = "checkpoints/rt_1_tf_trained_for_000400120"               # HF Hub Path (or path to local run directory)
